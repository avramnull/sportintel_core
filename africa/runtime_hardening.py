...
def _team_form_elo(hist: pd.DataFrame, team: str, as_of=None) -> dict:
    out = {f"FormPts_{w}": np.nan for w in (5,10,20)}; out.update({f"FormGD_{w}":np.nan for w in (5,10,20)}); out.update({"Elo":1500.0,"TeamId":-1})
    ...
    out["Elo"] = float(elo.get(tid,1500.0)); sub=h[(h["HomeTeam"]==canon)|(h["AwayTeam"]==canon)].tail(20); pts=[]; gd=[]
    for _,r in sub.iterrows():
        home=str(r["HomeTeam"])==canon; ftr=str(r.get("FTR","D")).upper(); pts.append(3 if (ftr=="H" if home else ftr=="A") else (1 if ftr=="D" else 0)); gd.append(float(r["FTHG"]-r["FTAG"]) if home else float(r["FTAG"]-r["FTHG"]))
    for w in (5,10,20):
