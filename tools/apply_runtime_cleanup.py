...
repl("train.py", 'focus_list: List[Optional[str]] = [None]', 'focus_list: List[Optional[str]] = []', required=False)
...
# 4) Remove obsolete external-provider client and standalone standings fetcher.
...
# 6) Scrub stale provider naming and fail only on executable references.
...
# One-time migration helper: remove itself after successful execution.
try:
